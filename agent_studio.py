...
GEMMA_ID = "mlx-community/gemma-4-e2b-it-8bit"
...
def _load_falcon():
    ...
    # torchvision >= 0.18 moved read_file out of the image C++ namespace.
    # Patch it back so Falcon Perception's batch_inference can find it.
    try:
        import sys, types, torchvision.io as _tio
        if not hasattr(_tio.image, 'read_file') and hasattr(_tio, 'read_file'):
            _fake = types.ModuleType('torchvision.io.image')
            _fake.read_file = _tio.read_file
            for _attr in ('decode_image', 'encode_jpeg', 'encode_png', 'write_jpeg', 'write_png'):
                _v = getattr(_tio, _attr, None)
                if _v is not None:
                    setattr(_fake, _attr, _v)
            sys.modules['torchvision.io.image'] = _fake
            _tio.image = _fake
    except Exception:
        pass
...
STEP_META = {
    ...
    "VLM":         {"icon": "🧠", "color": "#8b5cf6", "model": "Gemma 4 E2B",       "model_size": "2B",   "task": "Visual Reasoning"},
    "VLM_PLAN":    {"icon": "🤔", "color": "#a78bfa", "model": "Gemma 4 E2B",       "model_size": "2B",   "task": "Re-planning"},
